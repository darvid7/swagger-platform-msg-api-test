...
# create an instance of the API class
api_instance = swagger_client.MessagingApi(swagger_client.ApiClient(configuration))
payload = swagger_client.SendSMSRequest() # SendSMSRequest | A JSON or XML payload containing the recipient's phone number and text message.  This number can be in international format if preceeded by a â€˜+â€™ or in national format ('04xxxxxxxx') where x is a digit.
